...
try {
    ...
    $logStmt = $pdo->prepare("INSERT INTO delete_logs (tool_id, tool_name, action_type, deleted_by_id, deleted_by_name) VALUES (?, ?, 'restore', ?, ?)");
    $logStmt->execute([$id, $tool['tool_name'], $userId, $userName]);
    
    AuditLogger::log('tool_restored', [
        'tool_id' => $id,
        'tool_name' => $tool['tool_name']
    ], $userId, $_SESSION['user_email'] ?? null);
    
    $stmt = $pdo->prepare("UPDATE tools SET deleted_at = NULL WHERE id = ? AND deleted_at IS NOT NULL");
    $stmt->execute([$id]);
    ...
    error_log('restoreTool error: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => 'Failed to restore tool']);
}